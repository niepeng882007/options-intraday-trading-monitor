...
- **`src/main.py`** — `OptionsMonitor` orchestrator. Runs three APScheduler polling jobs: stock quotes (10s), option chains (30s), 1-minute bars (60s). Supports Futu OpenD (primary, real-time push) and Yahoo Finance (fallback). Data source configured via `data_source` in `config/settings.yaml`.
- **`src/collector/`** — `BaseCollector` ABC with `FutuCollector` (primary) and `YahooCollector` (fallback) implementations. Returns `StockQuote`, `OptionQuote`, and bar DataFrames.
- **`src/indicator/engine.py`** — `IndicatorEngine` manages per-symbol bar data across timeframes (1m, 5m, 15m via resampling). Calculates RSI, MACD, EMA (9/21/50/200), ATR, VWAP, Bollinger Bands, ADX. Returns `IndicatorResult` dicts.
...
- **`src/strategy/matcher.py`** — `RuleMatcher` evaluates entry/exit rules against indicators. Supports comparators (`>`, `<`, `crosses_above`, `within_pct_of`), nested AND/OR groups, entry quality scoring (0-100, grades A-D), `confirm_bars` multi-bar confirmation, `min_magnitude` threshold, and `market_context_filters`.
...
- **`src/backtest/`** — Backtesting framework. Replays historical bar data through IndicatorEngine + RuleMatcher to evaluate strategy performance. Outputs win rate, profit factor, max drawdown, equity curves. Run via `python -m src.backtest`.
