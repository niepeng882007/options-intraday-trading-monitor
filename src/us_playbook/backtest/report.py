"""Report generation for US backtest results."""
...
from src.us_playbook.backtest import USBacktestResult, LevelEvalResult, RegimeEvalResult, SimResult


def format_report(
    result: USBacktestResult,
    title: str = "US Predictor Backtest Report",
    period: str = "",
    verbose: bool = False,
) -> str:
...
def _format_level_section(level: LevelEvalResult, verbose: bool) -> list[str]:
    """Format Section 1: Level Signal Accuracy (VAH/VAL/PDH/PDL)."""
    lines = [
    ...
    # Header
    lines.append(
        f"  {'Threshold':<10} {'VAH T/B':>10} {'VAL T/B':>10} "
        f"{'PDH T/B':>10} {'PDL T/B':>10} {'Total Rate':>10}"
    )
    ...
    for threshold in sorted(level.by_threshold.keys()):
        ...
        for lk in level_keys:
            t = stats.get(f"{lk}_t", 0)
            b = stats.get(f"{lk}_b", 0)
            total_t += t
            total_b += b
            if t > 0:
                parts.append(f"{t}/{b}")
    ...
    # Per-level bounce rates at mid threshold
    if level.by_threshold:
        mid_threshold = sorted(level.by_threshold.keys())[len(level.by_threshold) // 2]
        stats = level.by_threshold[mid_threshold]
        lines.append("")
        lines.append(f"  Bounce rates @ {mid_threshold*100:.1f}% threshold:")
        for lk in level_keys:
            t = stats.get(f"{lk}_t", 0)
            b = stats.get(f"{lk}_b", 0)
            rate = f"{b/t*100:.1f}%" if t else "N/A"
            lines.append(f"    {lk.upper():<6} {t:>4} touches  {rate:>6} bounce rate")
    ...
    if verbose and level.by_symbol:
        ...
        for sym in sorted(level.by_symbol.keys()):
            ...
            if mid_threshold in sym_data:
                ...
                lines.append(
                    f"    {sym:<10} {s['touches']}T  {rate} (@ {mid_threshold*100:.1f}%)"
                )
...
def _format_regime_section(regime: RegimeEvalResult, verbose: bool) -> list[str]:
    """Format Section 2: Regime Classification Accuracy (D3 scoring)."""
    lines = [
    ...
    lines.append(f"  {'Regime':<14} {'Days':>6} {'Scorable':>9} {'Accurate':>9} {'Rate':>8}")
    ...
    for regime_type in sorted(regime.by_regime.keys()):
        ...
        if regime_type == "unclear":
            # UNCLEAR: not scored (D3)
            lines.append(
                f"  {regime_type:<14} {total:>6} {'':>9} {'':>9} {'(n/a)':>8}"
            )
        else:
            rate = f"{accurate/scorable*100:.1f}%" if scorable else "N/A"
            lines.append(
                f"  {regime_type:<14} {total:>6} {scorable:>9} {accurate:>9} {rate:>8}"
            )

    # Overall (excluding UNCLEAR)
    total_scorable = sum(s["scorable"] for s in regime.by_regime.values())
    total_accurate = sum(s["accurate"] for s in regime.by_regime.values())
    if total_scorable:
        lines.append(
            f"  {'OVERALL':<14} {'':>6} {total_scorable:>9} {total_accurate:>9} "
            f"{total_accurate/total_scorable*100:.1f}%"
        )
    ...
    if verbose and regime.by_symbol:
        ...
        for sym in sorted(regime.by_symbol.keys()):
            ...
            accurate = stats["accurate"]
            rate = f"{accurate/scorable*100:.1f}%" if scorable else "N/A"
            lines.append(f"    {sym:<10} {total}D  {scorable} scorable  {rate} accuracy")
    ...
    if verbose and regime.days:
        ...
        lines.append(
            f"  {'Date':<12} {'Symbol':<10} {'Regime':<14} {'Conf':>5} "
            f"{'RVOL':>5} {'Gap%':>6} {'O':>9} {'H':>9} {'L':>9} {'C':>9} {'OK?':>4}"
        )
        for d in regime.days:
            ...
            if not d.scorable:
                ...
                ok = "N"
            lines.append(
                f"  {date_str:<12} {d.symbol:<10} {d.predicted.value:<14} "
                f"{d.confidence:>5.2f} {d.rvol:>5.2f} {d.gap_pct:>+5.1f}% "
                f"{d.day_open:>9.2f} {d.day_high:>9.2f} "
                f"{d.day_low:>9.2f} {d.day_close:>9.2f} {ok:>4}"
            )
...
def _format_sim_section(sim: SimResult, verbose: bool) -> list[str]:
    ...
    if verbose and sim.trades:
        ...
        lines.append(
            f"  {'#':<3} {'Symbol':<10} {'Signal':<22} "
            f"{'Entry':>9} {'Exit':>9} {'Net%':>8} {'Peak%':>7} {'Reason'}"
        )
        for i, t in enumerate(sim.trades, 1):
            peak_str = f"{t.peak_pnl_pct:>+6.2f}%" if t.peak_pnl_pct else "     - "
            lines.append(
                f"  {i:<3} {t.symbol:<10} {t.signal_type:<22} "
...
def format_csv(result: USBacktestResult) -> str:
...
def format_json(result: USBacktestResult) -> str:
