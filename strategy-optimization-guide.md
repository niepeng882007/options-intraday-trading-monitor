...
| 绩效统计 | ✅ 回测报告 | 胜率、盈亏比、利润因子、最大回撤 — 见 `src/backtest/` |
| 回测框架 | ✅ 已实现 | 见 `src/backtest/` 和 `docs/backtest.md` |
| 宏观过滤 | ✅ 多层过滤 | `market_context_filters`、午间禁交易、每日亏损熔断 |
...
> ✅ **已实现** — `confirm_bars` 字段，支持在 YAML 规则中配置多 bar 确认
...
### Phase 5：精细化优化（持续迭代） — 部分完成

| 任务 | 说明 |
|------|------|
| ✅ 实施宏观过滤层 | `market_context_filters` 已实现 |
| ✅ 实施 Trailing Stop | bb-squeeze-ambush 已使用 trailing_stop |
| ✅ 实施信号确认等待期 | `confirm_bars` 字段已实现 |
| BBW 阈值按标的差异化 | 在 YAML 中支持 per-symbol 配置 |
| ✅ 增加量能确认条件 | 策略三/四/五均已增加量能条件 |
...
| RSI 超卖阈值 | 30 | [25, 28, 30, 32, 35] | 策略三 |
| VWAP 乖离阈值 | -0.8% | [-0.8%, -1.0%, -1.2%, -1.5%] | 策略三 |
